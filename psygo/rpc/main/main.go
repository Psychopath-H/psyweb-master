...
func (f Foo) Error(args Args, reply *int) error {
...
func startServer(registryAddr string, wg *sync.WaitGroup) { // registryAddr -> "http://localhost:9999/_rpc_/registry"
	...
	//限流
	server.SetLimiter(10, 100, time.Millisecond*time.Duration(200))

	//熔断
	settings := breaker.Settings{} //熔断器设置
	settings.Fallback = func(err error) (any, error) { //降级实现
