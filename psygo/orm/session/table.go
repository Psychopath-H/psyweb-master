...
func (s *Session) CreateTable() error {
	...
	_, err := s.Raw(fmt.Sprintf("CREATE TABLE %s (%s);", table.Name, desc)).Exec() //CREATE TABLE user (Name varchar(255) PRIMARY KEY,Age integer);
	return err
}

// DropTable drops a table with the name of model
func (s *Session) DropTable() error {
	_, err := s.Raw(fmt.Sprintf("DROP TABLE IF EXISTS %s", s.RefTable().Name)).Exec() //DROP TABLE IF EXISTS user
	return err
}
